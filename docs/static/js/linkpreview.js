const getOpenGraph = async (url) => {
    if (!window.$docsify.linkpreview.key)
      throw new Error(
        `Please get a key on linkpreview.net and add it as a configuration in $docsify.linkpreview.key as follows:
  ...
  const linkPreviewPlugin = (hook, vm) => {
  ...
  window.$docsify.plugins = (window.$docsify.plugins || []).concat(linkPreviewPlugin);